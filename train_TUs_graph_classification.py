...
"""
    For GCNs
"""
def train_epoch_sparse(model, optimizer, device, data_loader, epoch,net_params):
    model.train()
    epoch_loss = 0
    epoch_contras_loss,epoch_cls_loss=0,0
    epoch_train_acc,epoch_train_acc_homo,epoch_train_acc_hetero = 0,0,0
    nb_data = 0
    gpu_mem = 0
    for iter, (batch_graphs, batch_labels) in enumerate(data_loader):
        ...
        # epoch_train_acc_homo += accuracy(score_homo, batch_labels)
        # epoch_train_acc_hetero += accuracy(score_hetero, batch_labels)
        nb_data += batch_labels.size(0)

    epoch_loss /= (iter + 1)
    epoch_train_acc /= nb_data
    if model.name == 'cate' and net_params['cate_contras'] == True:
        epoch_contras_loss /= (iter + 1)
        epoch_cls_loss/= (iter + 1)
        return epoch_loss,epoch_cls_loss, epoch_contras_loss, epoch_train_acc, optimizer
    else:
        return epoch_loss, epoch_train_acc, optimizer

    

def evaluate_network_sparse(model, device, data_loader, epoch, net_params):
    model.eval()
    epoch_test_loss = 0
    epoch_test_acc = 0
    ...
    if model.name == 'cate' and net_params['cate_contras'] == True:
        epoch_contras_loss /= (iter + 1)
        epoch_cls_loss /= (iter + 1)
        return epoch_test_loss,epoch_cls_loss, epoch_contras_loss,epoch_test_acc
    else:
