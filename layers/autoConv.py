...
class AUTOGCNLayer(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, activation, dropout, graph_norm, batch_norm, K=8, num_filters=1, residual=False, gate=True, opt='single'):
        super().__init__()
    ...
    def forward(self, g, feature, snorm_n=None):
        h = self.mp(g, feature)
        out_low = []
        out_mid = []
        out_high = []
